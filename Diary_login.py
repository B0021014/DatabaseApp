    ...
    def connect2(self):
        ...
        try:
            ...
            with connection.cursor() as cursor:
                ...
                sql1 = "select * from user_info where (username, password) = ('{}', '{}');".format(self.name_entry.get(), self.pass_entry.get())
                cursor.execute(sql1)
                tmp = cursor.fetchall()
...
#check = "SELECT * FROM user_info WHERE username = '{}', password = '{}'".format(username, password)
...
'''
    ...
    def save(self, username, password):
        ...
        try:
            ...
            with self.connection.cursor() as self.cursor:
                # 取得した情報を DB に追記
                save = "INSERT INTO user_info (username, password) VALUES('{}', '{}')".format(username, password)
...
#'''
    ...
    def add(self):
        if self.sub_win == None or not self.sub_win.winfo_exists():
            ...
            '''# User
            label_2 = tk.Label(sb2_frame, text = 'User:')
            label_2.grid(row = 0, column = 1)
            label_3 = tk.Label(sb2_frame, text = self.connect())
